		...
		for (Map.Entry<Address,Command> entry : mod.m_code.entrySet()) {
			...
			if ( !cmd.isDefined() ){
				...
				if (new_value == null) {
					...
					Tuple<String,String> present = extern_names.stream().filter(t -> t.a.equals(cmd.getUndefValue())).findAny().orElseThrow(() -> { return new ExecutionException("Undefined symbol not (\"" + cmd.getLabel() + "\") present in extern directive");});
